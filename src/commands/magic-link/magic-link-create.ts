...
import { createMagicLink, getMagicLinkByCode } from "../../service/MagicLink.service";
...
export const MagicLinkCreateSubcommand: SlashCommandSubcommand = {
	slashCommandBuilder: new SlashCommandSubcommandBuilder()
        ...
        .addStringOption((option) => 
            option
                ...
                .setDescription("Paste this Discord server's invitation link here")
                .setRequired(true)
        )
        .addRoleOption((option) => 
            option
                .setName("role")
                .setDescription("The role to give to the user when they join with this link")
                .setRequired(true)
        ),
	...
	async onCommandExecuted(interaction) {
		...
		try {
            ...
            interaction.reply({
                content: `This invitation link (\`${code}\`) is already in use as the Magic Link`,
                embeds: [GuildMagicLinkListEmbed({ magicLinkList: [magicLink] })],
                ephemeral: true,
            });
            return;
        }
